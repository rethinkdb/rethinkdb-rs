...
* `wait_for`: a string indicating a table [status](/api/python/status) to wait on before returning. The default is `ready_for_writes`. 
...
See [status](/api/python/status) and [System tables](/docs/system-tables/) for a discussion of the fields within the `status` rows.
