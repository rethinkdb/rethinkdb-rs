...
- `batchConf`: specify new parameters to configure result batching, useful for larger result sets. `batchConf` takes a key/value array as its argument, letting you specify one or more of the parameters separately (i.e., `{batchConf: {max_batch_rows: 20}}`). Note that these parameters are not camelCased.
    ...
    - `first_batch_scaledown_factor`: factor to scale the other parameters down by on the first batch (default: 8). For example, with this set to 8 and `max_batch_rows` set to 8, on the first batch `max_batch_rows` will be adjusted to 1 (8 / 8). This allows the first batch to return faster.
...
```js
r.table('marvel').run(conn, {batchConf: {
    max_batch_rows: 16,
